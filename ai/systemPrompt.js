...
exports.buildInterventionMessages = buildInterventionMessages;
exports.parseActionSteps = parseActionSteps;
function detectMode(message, urgencyScore, context) {
    ...
    const vulnerabilityMatch = vulnerabilityKeywords.some((kw) => lowerMsg.includes(kw));
    if (urgeMatch || urgencyScore >= 7)
        return "URGE";
    if (vulnerabilityMatch || urgencyScore >= 4)
...
function buildInterventionMessages(userMessage, mode, context) {
    return [{ role: "user", content: userMessage }];
}
function parseActionSteps(rawMessage) {
